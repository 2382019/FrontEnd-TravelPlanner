import { useQuery } from '@tanstack/react-query';
import { Link } from 'react-router-dom';
import { postsAPI, budgetAPI, packingAPI, itineraryAPI, culinaryAPI } from '../utils/api';

export function Dashboard() {
  const { data: postsResponse } = useQuery({
    queryKey: ['posts'],
    queryFn: postsAPI.getAll,
  });

  const { data: budgetsResponse } = useQuery({
    queryKey: ['budgets'],
    queryFn: budgetAPI.getAll,
  });

  const { data: packingResponse } = useQuery({
    queryKey: ['packing-items'],
    queryFn: packingAPI.getAll,
  });

  const { data: itinerariesResponse } = useQuery({
    queryKey: ['itineraries'],
    queryFn: itineraryAPI.getAll,
  });

  const { data: culinaryResponse } = useQuery({
    queryKey: ['culinary-items'],
    queryFn: culinaryAPI.getAll,
  });

  const posts = postsResponse?.data || [];
  const budgets = budgetsResponse?.data || [];
  const packingItems = packingResponse?.data || [];
  const itineraries = itinerariesResponse?.data || [];
  const culinaryItems = culinaryResponse?.data || [];

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {/* Posts Summary */}
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <svg
                  className="h-6 w-6 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"
                  />
                </svg>
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="truncate text-sm font-medium text-gray-500">Total Posts</dt>
                  <dd className="text-lg font-medium text-gray-900">{posts.length}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 px-5 py-3">
            <div className="text-sm">
              <Link
                to="/posts"
                className="font-medium text-indigo-600 hover:text-indigo-500"
              >
                View all posts
              </Link>
            </div>
          </div>
        </div>

        {/* Budget Summary */}
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <svg
                  className="h-6 w-6 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                  />
                </svg>
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="truncate text-sm font-medium text-gray-500">Total Budget Items</dt>
                  <dd className="text-lg font-medium text-gray-900">{budgets.length}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 px-5 py-3">
            <div className="text-sm">
              <Link
                to="/budget"
                className="font-medium text-indigo-600 hover:text-indigo-500"
              >
                View all budget items
              </Link>
            </div>
          </div>
        </div>

        {/* Packing List Summary */}
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <svg
                  className="h-6 w-6 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"
                  />
                </svg>
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="truncate text-sm font-medium text-gray-500">Total Packing Items</dt>
                  <dd className="text-lg font-medium text-gray-900">{packingItems.length}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 px-5 py-3">
            <div className="text-sm">
              <Link
                to="/packing"
                className="font-medium text-indigo-600 hover:text-indigo-500"
              >
                View all packing items
              </Link>
            </div>
          </div>
        </div>

        {/* Itinerary Summary */}
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <svg
                  className="h-6 w-6 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
                  />
                </svg>
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="truncate text-sm font-medium text-gray-500">Total Itinerary Items</dt>
                  <dd className="text-lg font-medium text-gray-900">{itineraries.length}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 px-5 py-3">
            <div className="text-sm">
              <Link
                to="/itinerary"
                className="font-medium text-indigo-600 hover:text-indigo-500"
              >
                View all itinerary items
              </Link>
            </div>
          </div>
        </div>

        {/* Culinary Summary */}
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <svg
                  className="h-6 w-6 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"
                  />
                </svg>
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="truncate text-sm font-medium text-gray-500">Total Culinary Items</dt>
                  <dd className="text-lg font-medium text-gray-900">{culinaryItems.length}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 px-5 py-3">
            <div className="text-sm">
              <Link
                to="/culinary"
                className="font-medium text-indigo-600 hover:text-indigo-500"
              >
                View all culinary items
              </Link>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
} 